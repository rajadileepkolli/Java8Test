...
public class StreamDemo {
	...
	public static void main(String[] args) {
		...
		// Obtain the minimum and maximum value by use of
		// min(),max(),isPresent(),and get()
		...
		if (maxVal.isPresent()) {
			System.out.println("Maximum valu：" + maxVal.get());
		}
		...
		// Display only the odd values by use filter()
		Stream<Integer> oddVals = myList.stream().sorted().filter(n -> (n % 2) == 1);
		System.out.print("Odd values：");
		oddVals.forEach(n -> System.out.print(n + " "));
		System.out.println();

		// Display only the odd values that are greater than 5.Notice that two
		// filter operations are pipelined
